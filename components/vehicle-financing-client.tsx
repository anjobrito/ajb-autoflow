...
export function VehicleFinancingClient() {
  ...
  const filteredRecords = useMemo(() => records.filter((record) => {
    ...
    return matchesSearch(record, search) && matchesFinancing && matchesLien;
  }), [records, search, financingStatusFilter, lienStatusFilter]);
  ...
  const summary = useMemo(() => filteredRecords.reduce((acc, record) => {
    ...
    return acc;
  }, { contracts: 0, financed: 0, returnExpected: 0, returnReceived: 0, pendingLiens: 0, registeredLiens: 0 }), [filteredRecords]);
    ...
    <div className="grid gap-6">
      ...
      <form onSubmit={handleSubmit} className="rounded-3xl bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <button type="submit" className="rounded-2xl bg-blue-600 px-6 py-4 text-sm font-black text-white hover:bg-blue-700">{editingId ? "Salvar alterações" : "Cadastrar financiamento"}</button>
          {editingId ? <button type="button" onClick={resetForm} className="rounded-2xl border border-slate-300 px-6 py-4 text-sm font-black text-slate-700 hover:bg-slate-50">Cancelar edição</button> : null}
        </div>
      </form>
      ...
      <div className="rounded-3xl bg-white p-6 shadow-sm">
        <div className="grid gap-4 xl:grid-cols-[1fr_760px] xl:items-end">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <label className="relative"><Search className="absolute left-4 top-3.5 h-4 w-4 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Busca rapida" className={`${inputClass} pl-10`} /></label>
            ...
            <tbody>
              {filteredRecords.length > 0 ? filteredRecords.map((record) => (
                <tr key={record.id} className="border-b border-slate-100 align-top">
                  <td className="px-3 py-4"><p className="font-black">{record.customerName || "Cliente nao informado"}</p><p className="mt-1 text-slate-500">{record.customerDocument}</p><p className="text-slate-500">{record.customerPhone}</p></td>
                  <td className="px-3 py-4"><div className="flex items-start gap-2"><Car className="mt-0.5 h-4 w-4 text-blue-700" /><div><p className="font-black">{record.vehicleBrand} {record.vehicleModel}</p><p className="mt-1 text-slate-500">Placa: {record.vehiclePlate || "nao informada"}</p><p className="text-slate-500">Chassi: {record.vehicleChassis || "nao informado"}</p></div></div></td>
                  <td className="px-3 py-4"><p className="font-black">{record.financedBank || "Banco nao informado"}</p><p className="mt-1 text-slate-500">{record.contractNumber || "Contrato nao informado"}</p><p className="text-slate-500">{record.sellerName || "Sem vendedor"} • {record.branchName}</p></td>
