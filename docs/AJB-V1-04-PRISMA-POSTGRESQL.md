...
- `prisma/seed.js`: carga demo inicial para desenvolvimento local.
- `lib/db.ts`: client Prisma seguro para Next.js em desenvolvimento.
- `lib/tenant.ts`: helper temporário para recuperar a empresa demo.
- `lib/prisma-mappers.ts`: conversões entre enums técnicos do banco e labels em português usadas na UI.
